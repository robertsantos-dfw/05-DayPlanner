...
function html() {
  for (var i = 0; i < time.length; i++) {
    ...
    // div time    var b = document.createElement("div");    b.setAttribute("id", `time${time[i]}`);    b.setAttribute("class", "col-2");
    //div schedule value    var c = document.createElement("div");    c.setAttribute("id", `col${time[i]}`);    c.setAttribute("class", "col-8");
    //div button    var d = document.createElement("div");    d.setAttribute("class", "col-2");    var e = document.createElement("button");    e.setAttribute("class", `saveBtn${time[i]}`);    var f = document.createElement("img");    f.setAttribute("src", "img/Locked.svg.png");    f.setAttribute("alt", "");    f.setAttribute(      "style",      "width:100%;height:50px; padding: 0px; margin: 0px"    );
    //append to the container    e.append(f);    d.append(e);    a.append(b);    a.append(c);    a.append(d);    container.append(a);  }  calendar();  getFromLocalStorage();  setToLocalStorage();}html();
    //calendar functionfunction calendar() {  for (let c = 0; c < time.length; c++) {    let timeX = document.querySelector(`#time${time[c]}`);    let period = function () {      if (time[c] < 5 || time[c] == 12) {        return "[PM]";      } else {        return "[AM]";      }    };    let timeat = m.format(`${time[c]} ${period()}`).toString();    timeX.append(timeat);
